...
| MM-012 | High | QML serialized edit requests into helper arguments, and import paths or conversion server names could also enter process command lines. | Planning, apply, import, and conversion requests now use a bounded one-object stdin protocol; argv contains only fixed subcommands and opaque IDs. Legacy untrusted-data argv commands were removed and regression tests inspect both CLI and QML surfaces. |
| MM-013 | High | Reopening a validated source parent by pathname left intermediate symlink and regular-directory replacement windows in discovery and mutation. | Reads compare validated parent/source identities during no-follow opening; writes keep the parent descriptor through replacement; plans bind both source and parent device/inode identity. Adversarial discovery, symlink, and hardlink-backed regular-directory swap tests prove replacement content is rejected and neither target is changed. |
| MM-014 | Medium | Renaming a JSONC server reconstructed the member and could discard its internal comments or move it in the object. | The patcher now edits fields in place and replaces only the source-range key token. Regression coverage verifies comment retention and stable placement. |
| MM-015 | Medium | Gemini's selector accepted generic top-level `servers` and `mcp_servers` objects as MCP configuration. | Gemini now recognizes only `mcpServers`; unrelated objects are preserved and the documented object is created only for an explicit supported add operation. |
