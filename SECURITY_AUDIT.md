...
| MM-012 | High | QML serialized edit requests into helper arguments, exposing any user-entered secret material through process command lines before backend rejection. | Planning and apply requests now use a bounded one-object stdin protocol; argv contains only the fixed helper subcommand and opaque plan ID. The legacy JSON-in-argv commands were removed and regression tests inspect both CLI and QML surfaces. |
| MM-013 | High | Reopening a validated source parent by pathname left an intermediate-directory swap window. | Reads and writes now open every directory component with no-follow, fd-relative traversal and keep the resulting parent descriptor through target inspection and replacement. An adversarial intermediate-swap test proves neither the original nor attacker target is changed. |
